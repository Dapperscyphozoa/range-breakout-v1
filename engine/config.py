...
# 2026-05-12: auto-parse individual STRATEGY_FOO_BAR env vars into
# STRATEGY_PARAMS["foo_bar"]. Forks were silently losing operator-set knobs
# (e.g. STRATEGY_MIN_CLUSTER_PIVOTS) because only the JSON OVERRIDES env was
# parsed and fork config.py files didn't extend STRATEGY_PARAMS with those keys.
# Reserved env names (handled above) are skipped. Values are best-effort
# coerced to int → float → str.
_RESERVED_STRATEGY_ENVS = {
    "STRATEGY_TIMEFRAME",
    "STRATEGY_PARAMS_OVERRIDES",
    "STRATEGY_RECORDS_URL",  # used by some forks for backtest-records pulldown
}
def _coerce(v):
    s = v.strip()
    try: return int(s)
...
for _k, _v in os.environ.items():
    if not _k.startswith("STRATEGY_"): continue
    if _k in _RESERVED_STRATEGY_ENVS: continue
    _key = _k[len("STRATEGY_"):].lower()
    if not _key: continue
    # Don't clobber values already set by OVERRIDES JSON — OVERRIDES wins
    if _key in STRATEGY_PARAMS and _key in (
            json.loads(_overrides_raw).keys() if _overrides_raw else []):
        continue
    try:
        STRATEGY_PARAMS[_key] = _coerce(_v)
        _auto_applied.append(_key)
    except Exception as _e:
        print(f"[config] STRATEGY_{_key.upper()} parse failed: {_e}", flush=True)
if _auto_applied:
    print(f"[config] STRATEGY_* env auto-applied: {_auto_applied}", flush=True)
